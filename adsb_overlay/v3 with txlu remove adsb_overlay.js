    ...
    setTimeout(function () {
        ...
        async function fetchPlanes() {
            try {
                ...
                //const res = await fetch("https://uat.fullerton.w6mfb.com/skyaware978/data/aircraft.json");
                ...
                for (const plane of data.aircraft) {
                    ...
                    // --- Filter out flight numbers starting with 'TXLU' ---
                    if (plane.flight && plane.flight.trim().toLowerCase().startsWith("txlu")) {
                        continue;
                    }
        ...
        $('#openwebrx-map-extralayers').append(
            $('<label><input type="checkbox" ' +
                ...
                '<a href="https://github.com/manousosgrill/OpenwebRX_plugin_ADS-B_overlay" target="_blank" id="adsb-plane-link">' +
                'ADS-B Receiver</a> <span id="adsb-plane-count">(0)</span></label>')
